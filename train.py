...
def main():
    ...
    checkpoint_path = "D:/generative-inpainting-pytorch/images"
        ...
        for iteration in range(start_iteration, config['niter'] + 1):
            ...
            if iteration % (config['viz_iter']) == 0:
                ...
                if x.size(0) > viz_max_out:
                    viz_images = torch.stack([ground_truth, x[:viz_max_out], inpainted_result[:viz_max_out]], dim=1)
                else:
                    viz_images = torch.stack([ground_truth, x, inpainted_result], dim=1)
